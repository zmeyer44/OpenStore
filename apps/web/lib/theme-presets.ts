// Theme presets derived from shadcn/ui's design system.
// Base colors define the full neutral palette (background, foreground, cards, etc.).
// Accent colors override just the primary/chart/sidebar-primary variables.
// The final theme is: base color vars merged with accent color overrides.
...
// ── Base Colors ────────────────────────────────────────────────────────────
// These define the neutral tone / background palette.

export const BASE_COLORS: BaseColorPreset[] = [
  {
    name: "neutral",
    title: "Neutral",
    cssVars: {
...
// ── Accent Colors ──────────────────────────────────────────────────────────
// These override just the primary/chart/sidebar-primary variables on top of
// a base color. When name matches a base color name, it's excluded (base
// colors already set their own primary).
...
export interface FontOption {
  ...
  title: string;
  family: string;
  google?: string; // Google Fonts family name for loading
  type: "sans" | "serif" | "mono" | "display";
}

export const FONTS: FontOption[] = [
  { name: "geist", title: "Geist", family: '"Geist Sans", system-ui, sans-serif', type: "sans" },
  { name: "inter", title: "Inter", family: '"Inter", system-ui, sans-serif', google: "Inter:wght@400;500;600;700", type: "sans" },
  { name: "roboto", title: "Roboto", family: '"Roboto", system-ui, sans-serif', google: "Roboto:wght@400;500;700", type: "sans" },
  { name: "open-sans", title: "Open Sans", family: '"Open Sans", system-ui, sans-serif', google: "Open+Sans:wght@400;500;600;700", type: "sans" },
  { name: "lato", title: "Lato", family: '"Lato", system-ui, sans-serif', google: "Lato:wght@400;700", type: "sans" },
  { name: "poppins", title: "Poppins", family: '"Poppins", system-ui, sans-serif', google: "Poppins:wght@400;500;600;700", type: "sans" },
  { name: "raleway", title: "Raleway", family: '"Raleway", system-ui, sans-serif', google: "Raleway:wght@400;500;600;700", type: "sans" },
  { name: "dm-sans", title: "DM Sans", family: '"DM Sans", system-ui, sans-serif', google: "DM+Sans:wght@400;500;600;700", type: "sans" },
  { name: "figtree", title: "Figtree", family: '"Figtree", system-ui, sans-serif', google: "Figtree:wght@400;500;600;700", type: "sans" },
  { name: "source-sans", title: "Source Sans 3", family: '"Source Sans 3", system-ui, sans-serif', google: "Source+Sans+3:wght@400;500;600;700", type: "sans" },
  { name: "nunito", title: "Nunito", family: '"Nunito", system-ui, sans-serif', google: "Nunito:wght@400;500;600;700", type: "sans" },
  { name: "playfair", title: "Playfair Display", family: '"Playfair Display", Georgia, serif', google: "Playfair+Display:wght@400;500;600;700", type: "serif" },
  { name: "lora", title: "Lora", family: '"Lora", Georgia, serif', google: "Lora:wght@400;500;600;700", type: "serif" },
  { name: "merriweather", title: "Merriweather", family: '"Merriweather", Georgia, serif', google: "Merriweather:wght@400;700", type: "serif" },
  { name: "source-serif", title: "Source Serif 4", family: '"Source Serif 4", Georgia, serif', google: "Source+Serif+4:wght@400;500;600;700", type: "serif" },
  { name: "jetbrains-mono", title: "JetBrains Mono", family: '"JetBrains Mono", monospace', google: "JetBrains+Mono:wght@400;500;600;700", type: "mono" },
  { name: "fira-code", title: "Fira Code", family: '"Fira Code", monospace', google: "Fira+Code:wght@400;500;600;700", type: "mono" },
];

// "inherit" means heading uses the same font as body
export const HEADING_FONT_OPTIONS: FontOption[] = [
  { name: "inherit", title: "Same as body", family: "inherit", type: "sans" },
  ...FONTS,
];
...
export function getHeadingFont(name: string): FontOption | undefined {
  return HEADING_FONT_OPTIONS.find((f) => f.name === name);
}

/**
 * Returns Google Fonts URLs to load for the given config.
 */
export function getGoogleFontUrls(config: WorkspaceThemeConfig): string[] {
  const families: string[] = [];
  const body = getFont(config.bodyFont);
  if (body?.google) families.push(body.google);
  if (config.headingFont !== "inherit") {
    const heading = getFont(config.headingFont);
    if (heading?.google && heading.name !== body?.name) {
      families.push(heading.google);
    }
  }
...
export type MenuColorValue = (typeof MENU_COLORS)[number]["value"];
...
export type MenuAccentValue = (typeof MENU_ACCENTS)[number]["value"];
...
export function getBaseColor(name: string): BaseColorPreset | undefined {
  return BASE_COLORS.find((c) => c.name === name);
}

export function getAccentColor(name: string): AccentColorPreset | undefined {
  return ACCENT_COLORS.find((c) => c.name === name);
}

export function getRadius(name: string): (typeof RADII)[number] | undefined {
  return RADII.find((r) => r.name === name);
}

/**
 * Builds merged CSS variables for a given theme config.
 * Returns separate light and dark variable maps ready to inject.
 */
export function buildThemeCssVars(config: WorkspaceThemeConfig): {
  light: Record<string, string>;
  dark: Record<string, string>;
} {
  const base = getBaseColor(config.baseColor) ?? getBaseColor("zinc")!;
  const accent = getAccentColor(config.accentColor);
  const radius = getRadius(config.radius);

  const light: Record<string, string> = { ...base.cssVars.light };
  const dark: Record<string, string> = { ...base.cssVars.dark };

  // Overlay accent color overrides
  if (accent) {
    Object.assign(light, accent.cssVars.light);
    Object.assign(dark, accent.cssVars.dark);
  }

  // Chart color override (if different from accent)
  if (config.chartColor && config.chartColor !== config.accentColor) {
    const chartTheme = getAccentColor(config.chartColor);
    if (chartTheme) {
      for (let i = 1; i <= 5; i++) {
        const key = `chart-${i}`;
        if (chartTheme.cssVars.light[key]) light[key] = chartTheme.cssVars.light[key];
        if (chartTheme.cssVars.dark[key]) dark[key] = chartTheme.cssVars.dark[key];
      }
    }
  }
  ...
  // Menu accent: bold makes accent = primary
  ...
  // Menu color: inverted swaps sidebar to use dark values in light mode
  const isInverted =
    config.menuColor === "inverted" ||
  ...
  if (isInverted) {
    // In light mode, sidebar uses dark-like colors
    light.sidebar = dark.sidebar ?? dark.background;
    light["sidebar-foreground"] = dark["sidebar-foreground"] ?? dark.foreground;
    light["sidebar-primary"] = dark["sidebar-primary"];
    light["sidebar-primary-foreground"] = dark["sidebar-primary-foreground"];
    light["sidebar-accent"] = dark["sidebar-accent"];
    light["sidebar-accent-foreground"] = dark["sidebar-accent-foreground"];
    light["sidebar-border"] = dark["sidebar-border"] ?? dark.border;
    light["sidebar-ring"] = dark["sidebar-ring"] ?? dark.ring;
  }

  if (isTranslucent) {
    // Add transparency to sidebar background
    light.sidebar = light.sidebar.replace(")", " / 80%)").replace("/ 80%) / 80%)", "/ 80%)");
    dark.sidebar = dark.sidebar.replace(")", " / 80%)").replace("/ 80%) / 80%)", "/ 80%)");
  }
...
/**
 * Generates a CSS string from theme config that can be injected as a <style> tag.
 */
export function buildThemeStylesheet(config: WorkspaceThemeConfig): string {
  const { light, dark } = buildThemeCssVars(config);

  const toVarDecls = (vars: Record<string, string>) =>
    Object.entries(vars)
...
const p = (c: Partial<WorkspaceThemeConfig>): WorkspaceThemeConfig => ({
  ...DEFAULT_THEME_CONFIG,
  chartColor: c.accentColor ?? DEFAULT_THEME_CONFIG.accentColor,
  ...c,
});

export const THEME_PRESETS: ThemePreset[] = [
  { name: "default", title: "Default", config: p({}) },
  { name: "neutral-minimal", title: "Neutral Minimal", config: p({ baseColor: "neutral", accentColor: "blue", radius: "small" }) },
  { name: "stone-amber", title: "Stone Amber", config: p({ baseColor: "stone", accentColor: "amber", radius: "large" }) },
  { name: "mauve-purple", title: "Mauve Purple", config: p({ baseColor: "mauve", accentColor: "purple" }) },
  { name: "mauve-pink", title: "Mauve Pink", config: p({ baseColor: "mauve", accentColor: "pink", radius: "large" }) },
  { name: "olive-emerald", title: "Olive Emerald", config: p({ baseColor: "olive", accentColor: "emerald" }) },
  { name: "olive-lime", title: "Olive Lime", config: p({ baseColor: "olive", accentColor: "lime", radius: "small" }) },
  { name: "mist-cyan", title: "Mist Cyan", config: p({ baseColor: "mist", accentColor: "cyan" }) },
  { name: "mist-sky", title: "Mist Sky", config: p({ baseColor: "mist", accentColor: "sky", radius: "large" }) },
  { name: "taupe-orange", title: "Taupe Orange", config: p({ baseColor: "taupe", accentColor: "orange" }) },
  { name: "zinc-rose", title: "Zinc Rose", config: p({ baseColor: "zinc", accentColor: "rose" }) },
  { name: "zinc-violet", title: "Zinc Violet", config: p({ baseColor: "zinc", accentColor: "violet", radius: "large" }) },
  { name: "neutral-red", title: "Neutral Red", config: p({ baseColor: "neutral", accentColor: "red", radius: "none" }) },
  { name: "stone-green", title: "Stone Green", config: p({ baseColor: "stone", accentColor: "green" }) },
  { name: "zinc-indigo", title: "Zinc Indigo", config: p({ baseColor: "zinc", accentColor: "indigo" }) },
  { name: "taupe-yellow", title: "Taupe Yellow", config: p({ baseColor: "taupe", accentColor: "yellow", radius: "small" }) },
  { name: "editorial", title: "Editorial", config: p({ baseColor: "stone", accentColor: "red", bodyFont: "lora", headingFont: "playfair", radius: "small" }) },
  { name: "terminal", title: "Terminal", config: p({ baseColor: "neutral", accentColor: "green", bodyFont: "jetbrains-mono", headingFont: "inherit", radius: "none" }) },
  { name: "modern", title: "Modern", config: p({ baseColor: "zinc", accentColor: "violet", bodyFont: "poppins", headingFont: "inherit", radius: "large" }) },
];

/**
 * Returns a random theme config.
 */
export function getRandomThemeConfig(): WorkspaceThemeConfig {
  ...
  const chart = ACCENT_COLORS[Math.floor(Math.random() * ACCENT_COLORS.length)]!;
  const radius = RADII[Math.floor(Math.random() * RADII.length)]!;
  const font = FONTS[Math.floor(Math.random() * FONTS.length)]!;
  const menuColorOpts = MENU_COLORS[Math.floor(Math.random() * MENU_COLORS.length)]!;
  const menuAccentOpts = MENU_ACCENTS[Math.floor(Math.random() * MENU_ACCENTS.length)]!;
  // Bold accent is incompatible with translucent
  const resolvedAccent =
    menuAccentOpts.value === "bold" &&
    (menuColorOpts.value === "default-translucent" || menuColorOpts.value === "inverted-translucent")
      ? "subtle"
      : menuAccentOpts.value;
  return {
    baseColor: base.name,
    ...
    bodyFont: font.name,
    headingFont: "inherit",
    menuColor: menuColorOpts.value,
    menuAccent: resolvedAccent,
  };
}
