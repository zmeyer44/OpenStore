...
test.describe.serial('Core user flows', () => {
  ...
  test('register a new account', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/02-dashboard-after-register.png' });
  });
  ...
  test('sign in with existing account', async ({ page }) => {
    ...
    await page.waitForURL('/', { timeout: 15000 });
    await page.screenshot({ path: 'e2e/screenshots/04-dashboard-after-login.png' });

    await expect(page.getByText('OpenStore')).toBeVisible();
  ...
  test('create a folder', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/06-create-folder-dialog.png' });
  ...
  test('upload a file', async ({ page }) => {
    ...
    const buffer = Buffer.from('Hello, OpenStore! This is a test file.');
  ...
  test('share a file via context menu', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/15-share-link-created.png' });
  ...
  test('view share links page', async ({ page }) => {
    ...
    await page.locator('[data-sidebar="menu-button"]', { hasText: 'Share Links' }).click();
  ...
  test('view upload links page', async ({ page }) => {
    ...
    await page.locator('[data-sidebar="menu-button"]', { hasText: 'Upload Links' }).click();
  ...
  test('rename a file', async ({ page }) => {
    ...
    await page.waitForTimeout(2000);
    await expect(page.getByText('renamed-document.txt')).toBeVisible({ timeout: 5000 });
  ...
  test('mobile sidebar toggle works', async ({ page }) => {
    ...
    if (await trigger.isVisible()) {
      await trigger.click();
      await page.waitForTimeout(500);
      await page.screenshot({ path: 'e2e/screenshots/22-mobile-sidebar-open.png' });
    }
  });
});
